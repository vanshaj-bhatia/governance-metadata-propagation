...
class DocumentRAGEngine:
    def __init__(self, project_id: str, location: str = "us-central1", credentials: Optional[Any] = None):
        ...
        self.embedder = VertexAIEmbedder(project_id, location, credentials=self._credentials)
        self.chunks = []
        self.embeddings = []
        self._client = None
    ...
    def load_document(self, file_path: str, chunk_size: int = 1000, chunk_overlap: int = 200):
        ...
        if ext in [".txt", ".pdf", ".md"]:
            ...
            raise ValueError("DOCX files are not supported directly by Gemini in this setup. Please convert to PDF or TXT first.")
        else:
        ...
        if new_chunks:
            ...
            new_embeddings = self.embedder.get_embeddings(new_chunks, task_type="RETRIEVAL_DOCUMENT")
    ...
    def _extract_text_via_gemini(self, file_path: str) -> str:
        ...
        logger.info(f"Extracting structured governance data via Gemini: {file_path}")
        ...
        try:
            ...
            logger.info(f"=== DEBUG: Gemini Extraction took {duration:.2f} seconds ===")
            
            if hasattr(response, 'usage_metadata') and response.usage_metadata:
                logger.info(f"Tokens consumed: {response.usage_metadata.total_token_count}")
    ...
    def retrieve(self, query: str, top_k: int = 3) -> List[Dict[str, Any]]:
        ...
        query_emb = self.embedder.get_embedding(query, task_type="RETRIEVAL_QUERY")
        if not query_emb:
            return []
