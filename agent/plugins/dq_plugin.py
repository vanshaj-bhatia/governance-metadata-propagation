...
class DQPlugin(BasePlugin):
    ...
    def _check_completeness_bq(self, dataset_id: str, table_name: str, column_name: str) -> Optional[float]:
        """
        ...
        try:
            ...
            logger.debug(f"BQ Completeness fallback failed for {table_name}.{column_name}: {e}")
        return None

    def get_latest_dq_job(self, dataset_id: str, table_name: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def get_latest_profile_job(self, dataset_id: str, table_name: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            request = dataplex_v1.ListDataScanJobsRequest(parent=scan_name, page_size=1)
    ...
    def fetch_dq_summary(self, dataset_id: str, table_name: str, column_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if dq_job:
            ...
            if column_name:
                ...
                col_rules = [r for r in rules if r.get("rule", {}).get("column") == column_name]
                if col_rules:
                    passed_count = sum(1 for r in col_rules if r.get("passed", False))
        ...
        if profile_job:
            ...
            if column_name:
                ...
                col_profile = next((f for f in fields if f.get("name") == column_name), None)
                if col_profile:
        ...
        if column_name:
            bq_score = self._check_completeness_bq(dataset_id, table_name, column_name)
