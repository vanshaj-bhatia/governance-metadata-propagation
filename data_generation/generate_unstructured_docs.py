...
def create_pdf_docs():
    ...
    story.append(Paragraph("Data Security Policy: Financial Transactions", title_style))
    ...
    # Noise Paragraph 1
    story.append(Paragraph("2. [NOISE] Office Supplies Expense Guidelines", noise_section_style))
    story.append(Paragraph(
    ...
    story.append(Paragraph("3. Column Specifications and Classifications", section_style))
    story.append(Paragraph(
    ...
    data = [
        ...
        [
            ...
            Paragraph("Primary key. Unique payment confirmation number.", table_cell_style),
            Paragraph("None (Public Ref)", table_cell_style)
        ],
        [
            Paragraph("order_id", table_cell_style),
            Paragraph("STRING", table_cell_style),
            Paragraph("Foreign key linking the transaction to an order.", table_cell_style),
            Paragraph("None (Public Ref)", table_cell_style)
        ],
        [
            Paragraph("product_id", table_cell_style),
            Paragraph("STRING", table_cell_style),
            Paragraph("Foreign key referencing the purchased item.", table_cell_style),
            Paragraph("None (Public Ref)", table_cell_style)
        ],
        [
            Paragraph("quantity", table_cell_style),
            Paragraph("INTEGER", table_cell_style),
            Paragraph("Number of units purchased in the transaction.", table_cell_style),
            Paragraph("None (Metric)", table_cell_style)
        ],
        [
            Paragraph("amount", table_cell_style),
            Paragraph("FLOAT", table_cell_style),
            Paragraph("The financial value of the transaction in USD.", table_cell_style),
            Paragraph("Sensitive (Requires Financial Policy Tag)", table_cell_style)
        ]
    ]
    ...
    # Noise Paragraph 2
    story.append(Paragraph("4. [NOISE] Upcoming Office Holidays", noise_section_style))
    story.append(Paragraph(
    ...
    doc.build(story)
    print("Created unstructured_docs/transaction_security_policy.pdf (with noise)")
