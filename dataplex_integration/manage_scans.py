...
if not PROJECT_ID:
    print("⚠️ Warning: GOOGLE_CLOUD_PROJECT not found in environment or .env file.")
...
def create_dq_scan(table_name, credentials=None):
    ...
    try:
        ...
        if "fine-grained reader" in str(e).lower():
            print(f"❌ Permission Error: Dataplex service account needs access to policy tags.")
