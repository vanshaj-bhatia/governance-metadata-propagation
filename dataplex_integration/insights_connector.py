...
class DataInsightsClient:
    ...
    def _get_resource_fqn(self, dataset_id: str, table_id: Optional[str] = None) -> str:
        if table_id:
    ...
    def _get_scan_id(self, dataset_id: str, table_id: Optional[str] = None) -> str:
        if table_id:
            return f"doc-scan-{table_id.replace('_', '-')}"
        return f"doc-scan-dataset-{dataset_id.replace('_', '-')}"

    def ensure_publishing_labels(self, dataset_id: str, table_id: Optional[str] = None):
        ...
        try:
            ...
            print(f"Warning: Failed to update labels for {table_id or dataset_id}: {e}")

    def create_and_start_scan(self, dataset_id: str, table_id: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            data_scan.data.resource = self._get_resource_fqn(dataset_id, table_id)
            ...
            op = self.dataplex_client.create_data_scan(parent=parent, data_scan=data_scan, data_scan_id=scan_id)
            op.result()
    ...
    def wait_for_job(self, job_name: str, timeout_sec: int = 600) -> Optional[Dict[str, Any]]:
        """Polls a Dataplex job until completion and returns the FULL result dict."""
        print(f"Waiting for job {job_name.split('/')[-1]}...")
    ...
    def extract_insights(self, job_dict: Dict[str, Any], table_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    def run_full_sync(self, dataset_id: str, table_id: Optional[str] = None, output_file: str = "knowledge_insights.json"):
