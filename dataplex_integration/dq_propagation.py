...
from lineage_propagation import LineageGraphTraverser, SQLFetcher, TransformationEnricher

logger = logging.getLogger(__name__)

class DQPropagationEngine:
    def __init__(self, project_id: str, location: str, token: Optional[str] = None):
    ...
    def get_column_dq_lineage(self, target_fqn: str, columns: List[str], max_depth: int = 3) -> Dict[str, Any]:
        """
        Traverses upstream to find the DQ sources for each column (multi-hop).
        """
        return self._traverser.get_recursive_column_lineage(target_fqn, columns, max_depth=max_depth)

    def detect_remediation(self, target_dataset: str, target_table: str, column: str) -> float:
        """
        ...
        """
        sql = self._get_sql_fetcher().get_transformation_sql(target_dataset, target_table)
        if not sql:
            return 0.0
        ...
        if any(kw in logic_upper for kw in ["COALESCE", "IFNULL", "NULLIF", "CASE"]):
            bonus += 0.1
            
        # 3. Validity remediation
        if "SAFE." in logic_upper or "CAST(" in logic_upper or "REGEXP_REPLACE" in logic_upper:
            bonus += 0.05
            
        return min(bonus, 0.3) # Max bonus capped at 0.3

    def aggregate_scores(self, upstream_scores: List[float], method: str = "conservative_min") -> float:
        """
    ...
    def update_history(self, fqn: str, column: str, score: float, source_type: str = "DERIVED", dimensions: Dict[str, float] = None):
        ...
        try:
            ...
            if errors:
                logger.warning(f"Failed to insert row into BQ DQ history: {errors}")
    ...
    def propagate_dq_scores(self, target_fqn: str, dataset_id: str, table_name: str, columns: List[str]):
        ...
        for col in columns:
            ...
            if not all_mappings:
                ...
                results[col] = {"score": 0.0, "source_type": "UNKNOWN", "bonus": 0.0}
                continue
